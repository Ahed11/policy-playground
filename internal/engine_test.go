...
func TestCreateAlertErrors(t *testing.T){
	tests := []struct {
		...
		},
		{
			...
			"политика pol_external_client_data: поле не поддерживается",
		},
    }
...
func TestCreateAlert(t *testing.T) {
	...
	arrayOfForms := []formForTestCreateAlert{
		...
		},
		{
			...
			true,
			[]string{"file_ext does not exist"},
		},
		{
			"notSuccessExists",
			Event{
		...
		},
		{
			...
			true,
			[]string{"action equals email_send", "destination_type equals external", "content_classes contains personal_data", "file_ext in [xlsx docx pdf]", "file_ext exists"},
		},
		{
			"successAll_2",
			Event{
			...
			true,
			[]string{"action equals email_send", "destination_type equals external", "content_classes contains personal_data", "file_ext in [xlsx docx pdf]", "file_name does not exist"},
		},
		{
			"successAll_3",
			Event{
			...
			true,
			[]string{"action equals email_send", "destination_type equals external", "content_classes contains personal_data", "file_ext in [xlsx docx pdf]", "file_name exists"},
		},
		{
			"notSuccessAll_1",
			Event{
		...
		},
		{
			...
			true,
			[]string{"action equals email_send", "file_ext in [xlsx docx pdf]", "file_name exists"},
		},
		{
			"successAny_2",
			Event{
			...
			true,
			[]string{"action equals email_send", "file_name exists"},
		},
		{
			"successAny_3",
			Event{
			...
			true,
			[]string{"action equals email_send", "file_name does not exist"},
		},
		{
			"notSuccessAny",
			Event{
			...
			Policy{
				...
				Condition: Condition{
					Any: []Condition{
						...
						{Field: "content_classes", Contains: "personal_data"},
						{Field: "file_ext", In: []string{"xlsx", "docx", "pdf"}},
						{Field: "file_ext", Exists: new(false)},
					},
				},
			},
			false,
